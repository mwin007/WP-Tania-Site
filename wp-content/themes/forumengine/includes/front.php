...
class ET_ForumFront extends ET_ForumEngine{
	...
	public function __construct(){
		...
		$time_send_mail = apply_filters( 'fe_time_send_mail' , 900 );
	...
	public function on_add_scripts(){
		...
		$front_texts = array(
			...
			'texts' => array(
				...
				'login_2_view'    => __("You need to sign into your account view this thread.", ET_DOMAIN),
...
class ET_ForumFrontPost extends ET_Base{
	...
	public function handle_posts(){
		...
		if ( isset($_POST['fe_nonce']) && wp_verify_nonce( $_POST['fe_nonce'], 'insert_thread' )){
			...
			// if($captcha->et_checkCaptcha($content) !=1){
			// 	wp_redirect( home_url().'?error=202' );
			// 	exit;
			// }
...
HTML;
	...
	private function thread_template($thread){
		...
		$undo  =  sprintf( __('Want to %s ?',ET_DOMAIN) , '<a href="#" class="act-undo">' . __('undo', ET_DOMAIN) . '</a>' );
		...
		// sticky
		$sticky 	= '';
		if ( et_is_sticky_thread($thread->ID) ){
			$sticky .= '<a href="#" class="sticky-thread active"><span class="icon" data-icon="S"></span></a>';
			$sticky_home_class = '';
		}
		else {
			$sticky .= '<a href="#" class="sticky-thread"><span class="icon" data-icon="S"></span></a>';
			$sticky_home_class = 'collapse';
		}

		if ( et_is_sticky_thread($thread->ID, true) )
			$sticky .= ' <a href="#" class="sticky-thread-home active"><span class="icon" data-icon="G"></span></a> ';
		else
			$sticky .= ' <a href="#" class="sticky-thread-home '. $sticky_home_class .' "><span class="icon" data-icon="G"></span></a> ';

		$notpending = ($thread->post_status != 'pending') ? $sticky. '<a href="#" class="close-thread '.$isClosed.'" data-toggle="tooltip" title="'.__('Close', ET_DOMAIN).'"><span class="icon" data-icon="("></span></a>
		...
		$is_sticky = et_is_sticky_thread($thread->ID);
		$is_sticky_home = et_is_sticky_thread($thread->ID, true);

		$template = <<<HTML
<li class="{$isHightLight} thread-item" data-id="{$thread->ID}" data-cat="{$thread->thread_category[0]->slug}">
	{$avatar_thumb}
...
HTML;
	...
	public function report(){
		...
		try {
			...
			$result = FE_Threads::report($_POST['content']['id'], $_POST['content']['message']);
